...
with tf.summary.create_file_writer('../arch/hparam_tuning').as_default():
    hp.hparams_config(
...
def run(run_dir, hparams,x_train, y_train, x_val , y_val, num_units,dropout_rate,optimizer ,layers ):
    with tf.summary.create_file_writer(run_dir).as_default():
        ...
        accuracy = train_test_model(hparams, x_train, y_train, x_val , y_val , run_dir, num_units,dropout_rate,optimizer,layers )
    ...
    for dropout_rate in (HP_DROPOUT.domain.min_value, HP_DROPOUT.domain.max_value):
        for optimizer in HP_OPTIMIZER.domain.values:
            ...
            HP_DENSE_LAYERS:layers
                ...
                run('../arch/hparam_tuning/' + run_name, hparams, x_train , y_train , x_val, y_val,num_units,dropout_rate,optimizer,layers)
                session_num += 1